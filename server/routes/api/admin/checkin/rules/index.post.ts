...
export default defineEventHandler(async (event) => {
  ...
  const [result]: any = await db.query(
    `INSERT INTO t_checkin_rule (rule_type, streak_days, points_amount, points_type, name, sort_order)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [rule_type, rule_type === 1 ? 1 : Number(streak_days), Number(points_amount), points_type, name.trim(), Number(sort_order) || 0]
  )
