export default defineEventHandler(async event => {
  ...
  if (!body || !validId(body.defaultModelId) || !Array.isArray(body.rules) || body.rules.length > 200 || !Array.isArray(body.breedMappings) || body.breedMappings.length > 1000) fail()
  ...
  try {
    ...
    if (!valid(body.defaultModelId) || rules.some((r: any) => r.enabled && !valid(r.modelId))) throw createError({ statusCode: 400, message: '默认形象和启用规则必须选择有效的启用形象' })
    await db.query('UPDATE t_pet_model_assignment SET default_model_id=?,rules=?,breed_mappings=? WHERE id=1', [body.defaultModelId, JSON.stringify(rules), JSON.stringify(breedMappings)])
    await db.commit()
    return { success: true }
