...
export default defineEventHandler(async (event) => {
  ...
  const now = new Date()
  const monthParam = query.month ? String(query.month) : businessDate(now).slice(0, 7)
  const [year, month] = monthParam.split('-').map(Number)
  if (!/^\d{4}-\d{2}$/.test(monthParam) || !year || !month || month < 1 || month > 12) {
  ...
  const logMap = new Map<string, { streak: number; isMarkup: number }>(logs.map((r: any) => {
    // mysql2 DATE 列可能返回 Date 对象，统一转成本地 YYYY-MM-DD 字符串
    const key = r.checkin_date instanceof Date
      ? businessDate(r.checkin_date)
  ...
  const todayDate = new Date(today) // 本地午夜，用于日期大小比较
