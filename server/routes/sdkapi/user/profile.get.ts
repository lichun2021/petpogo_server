...
export default defineEventHandler(async (event) => {
  ...
  const [[row]]: any = await db.query(
    "SELECT id, phone, nickname, avatar, gender, DATE_FORMAT(birthday, '%Y-%m-%d') AS birthday, email, bio, plan_type, plan_expire_at, created_at FROM t_user WHERE id=? AND deleted=0",
    [user.userId]
  )
