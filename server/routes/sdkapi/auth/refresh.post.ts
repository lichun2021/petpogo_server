 ...
 * 注意：此处不能用 granwin_token（已过期），要用 refresh_token（30天有效）
 *
...
export default defineEventHandler(async (event) => {
  ...
  // ── 同步调用对方后台换新 Token ────────────────────────────────
  // 对方返回完整的登录信息（含新 granwin_token + 新 refresh_token + 新 AWS 凭证）
  ...
  // ── 写入新的 Redis Session ────────────────────────────────────
  const newSessionKey = tokenSessionKey(peerInfo.granwin_token)
