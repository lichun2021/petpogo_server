...
export default defineEventHandler(async (event) => {
  ...
  const [[pet]]: any = await db.query(
    'SELECT id,name,avatar,species,breed,gender,birthday,weight,bio,device_id,satiety,mood,cleanliness,background_id,model_id FROM t_pet WHERE id=? AND user_id=? AND deleted=0 LIMIT 1',
    [id, user.userId]
  )
