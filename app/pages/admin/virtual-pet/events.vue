  ...
  <div class="space-y-4">
    ...
    <div class="bg-white rounded-2xl border p-4 space-y-3" style="border-color: #f0e6d8">
      ...
      <div class="flex items-center gap-3 flex-wrap">
        <USelectMenu v-model="source" :options="sourceOptions" value-attribute="value" option-attribute="label" placeholder="事件来源" class="w-40" />
      ...
      <table v-else class="w-full text-sm">
        <thead>
          <tr class="bg-amber-50/50 border-b border-orange-100">
            <th class="text-left text-xs text-stone-500 font-medium py-3 px-4">来源</th>
            ...
            <th class="text-left text-xs text-stone-500 font-medium py-3 px-4">发生时间</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="row in list" :key="row.id" class="border-b border-stone-100 hover:bg-amber-50/30 transition-colors">
            <td class="py-3 px-4">
              <UBadge :label="sourceLabel(row.source)" :color="row.source === 'interaction' ? 'amber' : 'sky'" variant="subtle" size="xs" />
            </td>
            <td class="py-3 px-4 text-stone-700">{{ row.pet_name || '-' }}</td>
            <td class="py-3 px-4 text-xs text-stone-400 font-mono">{{ row.device_id || '-' }}</td>
            <td class="py-3 px-4">
              <p class="text-stone-800">{{ row.type_name || '-' }}</p>
              <p class="text-xs text-stone-400 font-mono">{{ row.type_code }}</p>
            </td>
            <td class="py-3 px-4 text-xs text-stone-400">{{ formatDate(row.occurred_at) }}</td>
          </tr>
        </tbody>
      </table>
