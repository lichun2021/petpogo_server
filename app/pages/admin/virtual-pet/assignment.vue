    ...
    <template v-else-if="!loadError">
      <section class="bg-white border border-stone-200 rounded-xl p-5 space-y-3">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <label>默认猫形象<select v-model="config.defaultCatModelId" class="admin-select field">
            <option value="">请选择猫 GLB</option><option v-for="m in available" :key="m.id" :value="m.id">{{ m.name }}</option>
          </select></label>
          <label>默认狗形象<select v-model="config.defaultDogModelId" class="admin-select field">
            <option value="">请选择狗 GLB</option><option v-for="m in available" :key="m.id" :value="m.id">{{ m.name }}</option>
          </select></label>
          <label>未知类型保底形象<select v-model="config.defaultModelId" class="admin-select field">
      ...
      <section class="bg-white border border-stone-200 rounded-xl p-5 space-y-3">
        ...
        <div v-for="(r, i) in config.rules" :key="r.id || i" class="border border-stone-200 rounded-xl p-4 space-y-3">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <label>分配形象<select v-model="r.modelId" class="admin-select field"><option value="">请选择</option><option v-for="m in models" :key="m.id" :value="m.id">{{ m.name }}{{ m.enabled ? '' : '（已停用）' }}</option><option v-if="r.modelId && !models.some(m => m.id === r.modelId)" :value="r.modelId">原形象已删除，请重新选择</option></select></label>
            <label>宠物类型<select v-model="r.species" class="admin-select field"><option value="all">不限</option><option value="cat">猫</option><option value="dog">狗</option></select></label>
            <label>品种（多个用逗号分隔）<input v-model="r.breedText" class="field" placeholder="留空表示不限" /></label>
            <label>性别<select v-model="r.gender" class="admin-select field"><option :value="null">不限</option><option :value="1">公</option><option :value="2">母</option></select></label>
          </div>
      ...
      <section class="bg-white border border-stone-200 rounded-xl p-5 space-y-3">
        ...
        <div v-for="(m, i) in config.breedMappings" :key="i" class="flex gap-3 items-center">
          ...
          <select v-model="m.species" class="admin-select field"><option value="cat">猫</option><option value="dog">狗</option></select>
      ...
      <section class="bg-white border border-stone-200 rounded-xl p-5 space-y-3">
        ...
        <div class="flex gap-3">
          <select v-model="preview.species" class="admin-select field"><option value="other">未知（按品种识别）</option><option value="cat">猫</option><option value="dog">狗</option></select>
...
async function save() {
  ...
  try {
    ...
    await load()
    toast.add({ title: '形象分配配置已保存', color: 'success' })
  } catch (e) { toast.add({ title: error(e), color: 'error' }) } finally { saving.value = false }
}
async function runPreview() {
  ...
  catch (e) { toast.add({ title: error(e), color: 'error' }) } finally { previewing.value = false }
}
onMounted(load)
</script>
<style scoped>
