<template>
  <div class="space-y-4">
    ...
    <div class="bg-white rounded-xl border p-4 flex flex-wrap items-center gap-3" style="border-color: #e7e5e4">
      ...
      <UButton label="返回规则配置" color="neutral" variant="outline" icon="i-heroicons-arrow-left" to="/admin/points/rules" />
    </div>
      ...
      <table v-else class="w-full text-sm">
        <thead>
          <tr class="bg-stone-50 border-b border-stone-200">
            ...
            <th class="text-left text-sm text-stone-500 font-medium py-3 px-4">方向</th>
            <th class="text-left text-sm text-stone-500 font-medium py-3 px-4">积分类型</th>
            <th class="text-left text-sm text-stone-500 font-medium py-3 px-4">数量</th>
            <th class="text-left text-sm text-stone-500 font-medium py-3 px-4">变动后余额</th>
            <th class="text-left text-sm text-stone-500 font-medium py-3 px-4">原因</th>
        ...
        <tbody>
          <tr v-for="row in list" :key="row.id" class="border-b border-stone-100 hover:bg-primary/5 transition-colors">
            ...
            <td class="py-3 px-4">
              <UBadge :label="row.direction === 1 ? '获得' : '消耗'" :color="row.direction === 1 ? 'success' : 'primary'" variant="subtle" size="xs" />
            </td>
