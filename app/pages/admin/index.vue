<template>
  <div class="space-y-5">
    ...
    <div class="grid grid-cols-2 xl:grid-cols-4 gap-4">
      ...
      >
        <div>
          <p class="text-xs text-stone-500 mb-1">{{ s.label }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
      ...
      <div class="bg-white rounded-xl border overflow-hidden" style="border-color: #e7e5e4; box-shadow: 0 1px 4px rgba(0,0,0,0.04)">
        <div class="flex items-center justify-between px-5 py-3.5 border-b" style="border-color: #e7e5e4">
          <h3 class="text-base font-semibold text-stone-700">设备在线状态</h3>
          <UBadge :label="`${onlineCount} 在线`" color="success" variant="subtle" size="xs" />
        </div>
        <div class="divide-y" style="divide-color: #faf6f2">
          <div v-if="!recentDevices.length" class="px-5 py-8 text-center text-sm text-stone-500">暂无设备数据</div>
          <div v-for="d in recentDevices" :key="d.mac" class="flex items-center gap-3 px-5 py-2.5">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-xs text-stone-500 truncate">{{ d.mac }}</p>
            </div>
            <UBadge :label="d.online_status ? '在线' : '离线'" :color="d.online_status ? 'success' : 'neutral'" variant="subtle" size="xs" />
          </div>
        </div>
      </div>
        ...
        <div class="divide-y" style="divide-color: #faf6f2">
          ...
          <div v-for="p in recentPosts" :key="p.id" class="flex items-start gap-3 px-5 py-2.5">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-xs text-stone-500 truncate mt-0.5">{{ p.content }}</p>
            </div>
            <UBadge
              :label="p.status === 1 ? '正常' : p.status === 2 ? '待审' : '违规'"
              :color="p.status === 1 ? 'success' : p.status === 2 ? 'warning' : 'error'"
